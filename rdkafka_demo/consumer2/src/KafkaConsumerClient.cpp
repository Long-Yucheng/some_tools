...
int KafkaConsumerClient::init(std::vector<std::string> topics, char *brokers, char *groupId, consumer_callback consumer_cb, void * param_cb)
{
    ...
    // add brokers
    if (rd_kafka_conf_set(kafka_conf, "bootstrap.servers", brokers, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        printf("Error: %s\n", errstr);
        rd_kafka_conf_destroy(kafka_conf);
        return -2;
    }

    ret_conf = rd_kafka_conf_set(kafka_conf, "queued.min.messages", "1000000", errstr, sizeof(errstr));
    if (ret_conf != RD_KAFKA_CONF_OK) { printf("Error: rd_kafka_conf_set() failed 1; ret_conf=%d; errstr:%s\n", ret_conf, errstr); return -2; }

    ret_conf = rd_kafka_conf_set(kafka_conf, "session.timeout.ms", "6000", errstr, sizeof(errstr));
    if (ret_conf != RD_KAFKA_CONF_OK) { printf("Error: rd_kafka_conf_set() failed 2; ret_conf=%d; errstr:%s\n", ret_conf, errstr); return -2; }

    // 设置消费组
    ret_conf = rd_kafka_conf_set(kafka_conf, "group.id", groupId, errstr, sizeof(errstr));
    if(ret_conf != RD_KAFKA_CONF_OK){printf("Error: rd_kafka_conf_set() failed 3; ret_conf=%d; errstr:%s\n", ret_conf, errstr); return -2;}

    if (rd_kafka_conf_set(kafka_conf, "auto.offset.reset", "largest", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        printf("%s\n", errstr);
        rd_kafka_conf_destroy(kafka_conf);
        return -2;
    }
